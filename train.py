...
def main(params):
    ...
    if params.fuse_model_name in ["AttH", "RefH"]:
        kge_model = KGEModel.create_from_saved_model(os.path.join(params.main_dir, f"experiments/kge_baselines/KGE_{params.fuse_model_name}_{params.dataset}.pt"))        
    else:
        kge_model = None

    graph_classifier = initialize_model(params, dgl_model, kge_model, params.load_model)
...
if __name__ == '__main__':
    ...
    # Fuse Experiment Params (with AttH and RefH)
    parser.add_argument("--fuse_model_name", "-fm", type=str, default="AttH",
                        help="Name of translational KG model to fuse with")
